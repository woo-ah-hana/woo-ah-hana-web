...
import { Member, registerCommunity } from '@/app/business/community/community.service';
...
export default function MemberInviteModal(){
    ...
    const handleInviteButton = async () => {
        // setIsOpen(false);
        inquiryMember(invitedMemberPhoneNumberRef.current?.value as string).then((res)=>{
          if(res.data){
            ...
            <DialogContent className='p-10'>
                <div className='flex flex-col gap-8'>
                    ...
                    <div className='grid grid-cols-[7fr_3fr] w-full'>
                      <div className='text-lg mt-1 font-semibold'>{inquiriedMember.name}</div>
                      {inquiriedMember.name==="존재하지 않습니다."?
                      <></>
                      :<AchromaticButton variant={'outline'} onClick={async ()=>{
                        if(community.id!=='0'){
                          await registerCommunity(inquiriedMember.id, community.id).then((response)=>{
                            if(response.data?.success){
                              window.location.reload()
                            }else{alert('모임통장 초대에 실패했습니다.')}
                          })
                        }else{return}
                      }}>
                        초대하기
                    ...
                    <div className='w-full flex items-center justify-between gap-4'>
                        ...
                        <AchromaticButton className='w-full' onClick={handleInviteButton}>조회하기</AchromaticButton>
                    </div>
                </div>
            </DialogContent>
        </Dialog>
